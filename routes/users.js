...
const db = require("../db"); // Make sure to import your database module
...
/** GET /:username/jobs =>
 *   { user: { jobs: [ jobId, jobId, ... ] } }
 *
 * Authorization required: admin or current user
 */

router.get("/:username/jobs", ensureAdminOrCorrectUser, async function (req, res, next) {
  try {
    const user = await User.get(req.params.username);
    return res.json({ user });
  } catch (err) {
